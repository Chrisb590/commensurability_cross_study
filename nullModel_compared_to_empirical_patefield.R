...
# Join Patefield results with metadata
results_df <- results_df %>%
...
# Long format for all Patefield null values (1000 rows per index)
...
# Both long formats of raw data and Patefield null corrected indices
...
################################################################################
#
# Figure S4: Boxplot of raw vs Patefield null model for each publication 
# grouping and the three indices of specialization, nestedness, and modularity
#
################################################################################
...
p1 <- ggplot(df_specialization, aes(x = Publication, y = value, fill = Type)) +
  ...
  scale_fill_manual(values = c("Empirical" = "#fcae91", "Patefield null" = "#9972af")) +
...
# Combine plots
p_combined <- p1 / p2 / p3 + plot_layout(guides = "collect") & theme(legend.position = "top")
...
################################################################################
#
# Table 3 and S4: Delta and Z indices of specialization, nestedness, and 
# modularity
#
################################################################################
...
# Publication-level standard deviation summaries for indices
...
################################################################################
#
# Table S3 and S5: Patefield null corrected indices of specialization, 
...
################################################################################
#
# Table S6: % difference between each empirical network and average of 1000 
